...
export const grokeryApi = store => next => action => {
    return api(store, next, action, CALL_GROKERY_API, GROKERY_API)
...
const api = (store, next, action, api, url) => {
    if (typeof action[api] === 'undefined') {
        return next(action)
    }

    const { types } = action[api]
    const [requestType, successType, failureType] = types
    ...
    const { endpoint, method, data, callback } = action[api]
